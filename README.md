...
Iterative optimization pipeline for generating MEDI. Video frames are passed to the model to compute predicted responses and the loss is backpropagated. Dark yellow path (pixel space): gradients update frames directly. Green path (latent space): gradients update latent tensors, which are then decoded to frames via a pre-trained autoencoder.
